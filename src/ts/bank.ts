...
class Bank implements IBank {
  ...
  createMarkupBank(): HTMLElement {
    ...
    this.getAmountClientsOwe("USD", "UAH").then(({ amount, numberDebtors }) => {
      (
  ...
  createMarkupClientCard(): HTMLElement {
    ...
    this.clients.forEach(
      ({ name, surname, id, registrationDate, accounts }) => {
        ...
        const clientAccounts: any = client.querySelector(".client-accounts");

        accounts.forEach((account) =>
          clientAccounts.appendChild(this.createMarkupClientAccount(account))
        );
  ...
  async getAmountClientsOwe(
    ...
    return this.clients.reduce((accumulator: any, { accounts }, index) => {
      ...
      const totalDebt = accounts.reduce((acc, account) => {
        ...
        if (typeAcc === "credit") {
          let loanAmount: number = account.creditLimit - account.balance.credit;
